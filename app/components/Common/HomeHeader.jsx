        ...
        <nav className="hidden lg:flex items-center gap-8">
          {navItems.map((item) => (
            <div key={item.label} className="relative group">
              <Link
                href={item.href}
                className={`flex items-center gap-1 text-lg font-medium ${textColor} hover:text-[#9DCC46] transition-colors py-4`}
              >
                {item.label}
        ...
        <div className="hidden lg:flex items-center gap-4">
          <Link
            ...
            className={`rounded-full px-6 py-2.5 text-base font-semibold transition-all flex items-center gap-2 bg-[#DCE25A] text-gray-800 hover:bg-[#7AB316] ${
              !isScrolled && "border border-white hover:bg-white"
            }`}
          >
            Get In Touch
        ...
        <div className="lg:hidden px-4 py-6 bg-white shadow-md max-h-screen overflow-y-auto">
          {navItems.map((item) => (
            <div key={item.label} className="border-b last:border-0">
              ...
              {item.subItems && (
                <div id={`mobile-sub-home-${item.label}`} className="hidden bg-gray-50 pl-6 pb-4">
                  {item.subItems.map((sub) => (
                    <Link
                      key={sub.label}
                      href={sub.href}
